...
async function sendEmail(payload: { name: string; email: string; subject: string; message: string }) {
  ...
  if (error) return { sent: false, reason: "resend-error" as const, detail: error.message };
...
export async function POST(req: Request) {
  ...
  // Persist + email in parallel.
  const [, emailResult] = await Promise.all([persistLocally(payload), sendEmail(payload)]);
  ...
  return NextResponse.json({
    ...
    note: emailResult.sent
      ? "Email delivered."
      : "Stored locally — owner will see it via logs. Configure RESEND_API_KEY for email delivery.",
  });
}
